...
export class StockfishEngine {
      ...
      try {
        ...
        this.worker.onmessage = e => {
          ...
          if (s.startsWith('bestmove ')) {
            ...
            if (request) { this.pending.delete(this.activeId); clearTimeout(request.timer); request.resolve({ bestmove: s.split(/\s+/)[1] || null, info: this.lastInfo }); }
          }
        };
  ...
  async analyze(fen, depth = 16) {
    ...
    const id = ++this.seq; this.activeId = id; this.lastInfo = null;
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => { if (!this.pending.has(id)) return; this.pending.delete(id); this.worker.postMessage('stop'); reject(new Error('분석 시간 초과')); }, 60000);
