...
export async function analyzeGame(game,{depth=16,onProgress}={}){const chess=new ChessCtor();const out=[];for(let i=0;i<game.sans.length;i++){const before=chess.fen();const turn=chess.turn();const best=await engine.analyze(before,depth);const beforeScore=normalizeScore(best.info,turn);const played=chess.move(game.sans[i],{sloppy:true});if(!played)continue;const after=await engine.analyze(chess.fen(),depth);const afterScore=normalizeScore(after.info, chess.turn());const playedScore=afterScore==null?null:-afterScore;const loss=beforeScore==null||playedScore==null?null:Math.max(0,beforeScore-playedScore);out.push({ply:i+1,fen:before,san:played.san,uci:moveToUci(played),bestmove:best.bestmove,bestScore:beforeScore,playedScore,cpLoss:loss,phase:inferPhase(chess),classification:classify(loss),category:classifyReason(chess,played,loss)});onProgress?.(i+1,game.sans.length)}return out}

export function inferPhase(c){const pieces=c.board().flat().filter(Boolean);const queens=pieces.filter(p=>p.type==='q').length;const minors=pieces.filter(p=>['b','n'].includes(p.type)).length;const nonPawns=pieces.filter(p=>['q','r','b','n'].includes(p.type)).length;const ply=c.history().length;if(ply<20 && (queens===2||c.board().flat().filter(p=>p?.type==='k').some(k=>k)))return 'opening';if(queens===0&&nonPawns<=5)return 'endgame';if(queens===0&&minors<=2)return 'endgame';return 'middlegame'}
...
export function classifyReason(c,m,loss){if(loss<50)return 'none';const legal=c.moves({verbose:true});const forcing=legal.filter(x=>['c','x'].includes(x.flags)||x.san?.startsWith('K')).length; if(forcing>0)return 'calculation / forcing moves';if(['q','r'].includes(m.piece?.type))return 'piece activity / plan';return 'calculation'}
